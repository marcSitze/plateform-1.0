...
export const createComment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const newpost = await postsService.updatePost(post, {
      comments: [...postToUpdate.comments, comment._id],
    });
...
export const getComments = async (req: Request, res: Response) => {
  try {
    ...
    if (comments.length === 0) {
      return SuccessHandler(res, httpStatus.NO_CONTENT, {
        msg: "not comments found...",
      });
    }
